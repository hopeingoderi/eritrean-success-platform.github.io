// student/app.js
// ============================================================
// Student Frontend (SPA) - works with your current index.html:
// <nav id="nav">
...
//
// Fixes:
// - Login/Register/Logout visibility (only shows correct buttons)
// - Courses rendering (no more "undefined")
// - Course sorting: Foundation -> Growth -> Excellence
// - Lesson page: Return + Back + Next + Save & Complete
...
// Grab nav buttons by their text (your HTML has no IDs)
...
// Supports both "#dashboard" and "#/dashboard"
function getRoute() {
...
function setRoute(route) {
  // keep your existing style: #dashboard
...
async function loadCourses() {
  ...
  // sort: foundation -> growth -> excellence (fallback if "order" missing)
...
function progressFor(courseId, lessonIndex) {
  return state.progressByCourse[courseId]?.byLessonIndex?.[lessonIndex] || { completed: false, reflectionText: "" };
}
...
async function renderDashboard() {
  ...
  document.getElementById("langEn").onclick = async () => { state.lang = "en"; await loadCourses(); renderDashboard(); };
  document.getElementById("langTi").onclick = async () => { state.lang = "ti"; await loadCourses(); renderDashboard(); };
  ...
  wrap.querySelectorAll("[data-open]").forEach(btn => {
    btn.onclick = () => setRoute(`course/${btn.getAttribute("data-open")}`);
  });
}

async function renderCourse(courseId) {
  if (!courseId) return setRoute("dashboard");
  ...
  document.getElementById("courseProgress").innerHTML = `
    <div class="small">Progress: <b>${done}</b> / ${total} (${pct}%)</div>
    <div class="progressWrap" style="margin-top:6px;"><div class="progressBar" style="width:${pct}%"></div></div>
  `;

  document.getElementById("lessonsWrap").innerHTML = lessons
    .slice()
    .sort((a, b) => a.lessonIndex - b.lessonIndex)
    .map(l => {
      const completed = !!pmap[l.lessonIndex]?.completed;
      return `
        <div class="card">
          <div class="row">
            <div>
              <div class="h2">${escapeHtml(l.title || "")}</div>
              <div class="small">Lesson ${l.lessonIndex + 1} ${completed ? "✅" : ""}</div>
            </div>
  ...
  document.querySelectorAll("[data-lesson]").forEach(btn => {
    btn.onclick = () => setRoute(`lesson/${courseId}/${btn.getAttribute("data-lesson")}`);
  });
}

async function renderLesson(courseId, lessonIndexStr) {
  const lessonIndex = Number(lessonIndexStr);
  if (!courseId || !Number.isFinite(lessonIndex)) return setRoute(`course/${courseId}`);
  ...
  document.getElementById("returnCourse").onclick = () => setRoute(`course/${courseId}`);
  ...
  document.getElementById("lessonProgressBar").innerHTML = `
    <div class="small">Course progress: <b>${done}</b> / ${total} (${pct}%)</div>
    <div class="progressWrap" style="margin-top:6px;"><div class="progressBar" style="width:${pct}%"></div></div>
  `;
  ...
  document.getElementById("prevBtn").onclick = () => prevExists && setRoute(`lesson/${courseId}/${lessonIndex - 1}`);
  document.getElementById("nextBtn").onclick = () => nextExists && setRoute(`lesson/${courseId}/${lessonIndex + 1}`);
...
// ---------------- GLOBAL FUNCTIONS for your HTML ----------------
